...
using System.Xml;
using Products = Sample.WebApi.Models.Products;
...
namespace Sample.WebApi.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ProductCategoryCount>>> GetCategoryCount()
        {
            List<ProductCategoryCount> catCountList = new List<ProductCategoryCount>();

            try
            {
                var catCount = from product in _app_BlazorDBContext.Products
                               join category in _app_BlazorDBContext.ProductCategories
                               on product.ProductCategory equals category.CategoryName
                               group product by category.CategoryName into a
                               select new ProductCategoryCount
                               {
                                   CategoryName = a.Key,
                                   TotalItem = a.Count()
                               };

                catCountList.AddRange(catCount);
                if (catCount == null)
                {
                    logger.LogError($"Record Not Found:{nameof(Get)}");
                    return NotFound();
                }
                return Ok(catCountList);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, $"Error performing GetProduct By Category Count in {nameof(GetCategoryCount)}");
                return Ok(catCountList);
            }
